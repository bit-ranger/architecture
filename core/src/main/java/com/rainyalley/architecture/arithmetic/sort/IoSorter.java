...
public class IoSorter {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(IoSorter.class);
    ...
    private byte[] delimiter = new byte[]{ 13, 10 };
     ...
     * @param workDir 临时目录
     * @param cleanTmp 排序完成后，是否清除临时文件
     */
    public IoSorter(
            Comparator<byte[]> comparator,
            int mergeWayNum,
            int initialChunkSize, int ioBufferSize, File workDir, boolean cleanTmp) throws IOException{
        ...
        Assert.notNull(workDir, "workDir can not be null");
...
//            }
    ...
    private List<Chunk> split(File file) throws IOException{
        ...
        try(BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file), ioBufferSize)){
            int rowNum = INITIAL_ROW_NUM;

            List<ByteBuffer> chunkRows = new ArrayList<>(initialChunkSize);

            List<Future<Chunk>> splitFutureList = new ArrayList<>();
            while (true){
                ByteBuffer line = readLine(bis, ByteBuffer.allocate(200), new byte[delimiter.length]);
...
//                            cr.sort(comparator);
    ...
    /**
     * 读取一行
     * 如果container无法容纳一行的内容，将会自动扩容为新的container
     * @param is
     * @param container
     * @return
     * @throws IOException
     */
    public ByteBuffer readLine(InputStream is, ByteBuffer container, byte[] peek) throws IOException{
        container.clear();
        int c = is.read();
        if (c == -1) {
            return container;
        }
        container.put((byte)c);
        ...
        for (;;) {
            ...
            if(container.position() >= container.limit()){
                ByteBuffer  newContainer = ByteBuffer.allocate(container.limit() * 2);
                container.rewind();
                newContainer.put(container);
                container = newContainer;
            }
            container.put((byte)c);

            //发现delimiter末尾
            if(c == delimiter[delimiter.length-1]){
                int wpos = container.position();

                //不完整则忽略
                if(wpos  < delimiter.length){
                    continue;
                }

                //读取末尾
                container.position(wpos - delimiter.length);
                container.get(peek);
    ...
    private File workDir(File originalFile){
        return new File(workDir.getPath() + File.separator + originalFile.getName());
    }
